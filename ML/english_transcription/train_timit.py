...
while global_step < total_steps:
    ...
    # Training
    print(f'Current Step: {global_step}. Total step: {total_steps}.')
    train_loss = []
    train_ler = []
    for batch_index,(batch_data,batch_label) in tqdm(enumerate(train_set),total=valid_step-1):
        ...
        if global_step % valid_step == 0:
            break
    print(f'Current Step: {global_step}. Current Train Avg loss: {sum(train_loss)/len(train_loss)}. Total step: {total_steps}.')
    print(f'Current Step: {global_step}. Current Train Avg ler: {sum(train_ler)/len(train_ler)}. Total step: {total_steps}.')
    # Validation
    dev_loss = []
    dev_ler = []
    for _,(batch_data,batch_label) in tqdm(enumerate(valid_set),total=len(valid_set)):
        batch_loss, batch_ler = batch_iterator(batch_data, batch_label, listener, speller, optimizer, 
                                               tf_rate, is_training=False, **conf['model_parameter'])
        dev_loss.append(batch_loss)
        dev_ler.extend(batch_ler)
    
    print(f'Current Step: {global_step}. Current Val Avg loss: {sum(dev_loss)/len(dev_loss)}. Total step: {total_steps}.')
    print(f'Current Step: {global_step}. Current Val Avg ler: {sum(dev_ler)/len(dev_ler)}. Total step: {total_steps}.')
